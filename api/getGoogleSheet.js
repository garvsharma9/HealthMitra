export default async function handler(req, res) {
    ...
    if (!SHEET_ID || !API_KEY) {
        return res.status(200).json({ 
            error: `Missing env vars for "${type}". Need VERCEL_${(type || '').toUpperCase()}_SHEET_ID and VERCEL_${(type || '').toUpperCase()}_API_KEY.`,
            values: null
        });
    }
    ...
    try {
        ...
        if (data.error) {
            return res.status(200).json({
                error: data.error.message || 'Google Sheets API error',
                values: null
            });
        }
        ...
        res.status(200).json({ error: err.message, values: null });
    }
}
